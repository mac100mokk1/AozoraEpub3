...
public class WebAozoraConverter
{
		...
		boolean convertUpdated, boolean convertModifiedOnly, boolean convertModifiedTail, int beforeChapter) throws IOException
	{
		...
		//プロトコルがhttpかhttpsかそれ以外か
		if (urlString.startsWith("http://")) {
			this.protocol = "http";
		} else if (urlString.startsWith("https://")) {
			this.protocol = "https";
		} else {
			LogAppender.println("不明なプロトコルです");
			this.protocol = "http";
		}
		//http://を除外した文字列で比較
	...
	private void printRuby(BufferedWriter bw, Element ruby) throws IOException
	{
		...
		if (rt.size() > 0) {
			if (rb.size() > 0) {
				...
				String string_ruby = ruby.toString();
				String regex = "<ruby>(.+)<rt>(.+)</rt></ruby>";
				Pattern p = Pattern.compile(regex);
				Matcher m = p.matcher(string_ruby);
				if (m.find()){
					//debug
					// LogAppender.println("|" + m.group(1) + "《" + m.group(2) + "》");
					bw.append('｜');
					printText(bw, m.group(1));
					bw.append('《');
					printText(bw, m.group(2));
					bw.append('》');
				} else {
					LogAppender.println("警告: 対応していないルビのパターンです" + string_ruby);
					printText(bw, string_ruby);
				}
// デバッグ用
//				LogAppender.println("string_ruby string is "+string_ruby);
			}
		}
	}
	...
	private void printImage(BufferedWriter bw, Element img, File imageOutFile) throws IOException
	{
		...
		// https対策 //から始まっている場合にurlの前に"<protocol>:"を追加する
		if (src.startsWith("//")) {
